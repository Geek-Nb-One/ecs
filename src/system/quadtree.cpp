...
Quadtree::Quadtree(float x, float y, float width, float height, int maxCollidersPerNode) : root(new QuadtreeNode(x, y, width, height)), maxCollidersPerNode(maxCollidersPerNode)
{
}
Quadtree::~Quadtree()
{
...
bool Quadtree::inNode(const QuadtreeNode *node, const QuadtreeCollider &a) const
{
    switch (a.type)
    {
    case QuadtreeCollider::Type::Rectangle:
        return inNodeRect(node, a.x, a.y, a.width, a.height);
    case QuadtreeCollider::Type::Circle:
        return inNodeCircle(node, a.x, a.y, a.radius);
    case QuadtreeCollider::Type::Point:
        return inNodePoint(node, a.x, a.y);
    default:
...
bool Quadtree::isColliding(const QuadtreeCollider &a, const QuadtreeCollider &b) const
{
    // Rectangle
    if (a.type == QuadtreeCollider::Type::Rectangle)
    {
        if (b.type == QuadtreeCollider::Type::Rectangle)
        {
            return isCollidingRectRect(a, b);
        }
        else if (b.type == QuadtreeCollider::Type::Circle)
        {
            return isCollidingRectCircle(a, b);
        }
        else if (b.type == QuadtreeCollider::Type::Point)
        {
            return isCollidingRectPoint(a, b);
        }
        else
        {
            assert(false && "Unknown QuadtreeCollider type for B");
        }
    }
    // Circle
    else if (a.type == QuadtreeCollider::Type::Circle)
    {
        if (b.type == QuadtreeCollider::Type::Circle)
        {
            return isCollidingCircleCircle(a, b);
        }
        else if (b.type == QuadtreeCollider::Type::Rectangle)
        {
            return isCollidingRectCircle(b, a);
        }
        else if (b.type == QuadtreeCollider::Type::Point)
        {
            return isCollidingCirclePoint(a, b);
        }
        else
        {
            assert(false && "Unknown QuadtreeCollider type for B");
    ...
    // Point-Point
    else if (a.type == QuadtreeCollider::Type::Point)
    {
        if (b.type == QuadtreeCollider::Type::Circle)
        {
            return isCollidingCirclePoint(b, a);
        }
        else if (b.type == QuadtreeCollider::Type::Rectangle)
        {
            return isCollidingRectPoint(b, a);
        }
        else if (b.type == QuadtreeCollider::Type::Point)
        {
            return isCollidingPointPoint(a, b);
        }
        else
        {
            assert(false && "Unknown QuadtreeCollider type for B");
    ...
    {
        assert(false && "Unknown QuadtreeCollider type for A");
    }
...
bool Quadtree::inNodeRect(const QuadtreeNode *node, float x, float y, float w, float h) const
{
    return !(node->x + node->width < x || x + w < node->x ||
             node->y + node->height < y || y + h < node->y);
}

bool Quadtree::inNodeCircle(const QuadtreeNode *node, float x, float y, float radius) const
{
...
bool Quadtree::inNodePoint(const QuadtreeNode *node, float x, float y) const
{
    ...
    while (!nodesToCheck.empty())
    {
        ...
        if (inNode(currentNode, area))
        {
            ...
            if (currentNode->isLeaf)
            {
                ...
                {
                    if (isColliding(collider.collider, area))
                    {
